function [test_set_predictions,chain_stats,MODEL,mxsx,mnmx,SIG2] = bayes_lm(data,test,options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a Bayesian linear model modified after the original code of the
% BMARS model by Denison et al. See chapters 3, 4 in "Bayesian methods for nonlinear classification 
% and regression (2002). Denison, Holmes, Mallick and Smith: published by Wiley. 
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%	USAGE:
%			the program simulates a Bayesian linear model assuming a Gaussian reponse variable 
...
%	    MODEL is an object containing the simulated model strucutres along the chain. 
%                 Each element of MODEL represents a chosen model of one
%                 iteration, with the following subfields:
%                      MODEL{i}.basis  
%                      MODEL{i}.beta      : sampled model coefficents
%                      MODEL{i}.beta_mean : mean model coefficients
%                      MODEL{i}.k         : the total number of chosen covariates
%                      MODEL{i}.var    : the inidces of the k chosen covrariates
%                      
%       mxsx are the means and std deviations for each covariate of the data
%       mnmx are the minimum an maxiun values of each covariate of the data
%       SIG2 is an array containg a sample of simulated sig^2 (error variance)
%%
...
if nargin==3 % if then using user defined options
   ...
   interaction=[0,1]; options.interaction = interaction; % see below
   order=[0 1]; options.order = order;% 
   k_max = 60; options.k_max = k_max;% see below
...
%probMatrix=zeros(size(data,2)-1,size(data,2)-1);
...
if STANDARDISE
   ...
   mx=zeros(1,p); sx=ones(1,p);
end
% Ouput the means, standard deivations, mimina and maxima of the covariates X
mxsx=[mx;sx];
mnmx=minmax(X')';
%%
...
Xt_mars(:,1)=1;
%%
curVarIdx=zeros(1,k_max+1);
curVarIdx_prop=zeros(1,k_max+1);
curVarIdx(1)=0;
curVarIdx_prop(1)=0;
%
% Recall.....
...
while sample < mcmc_samples
   ...
   elseif u < 0.66
      ...
      move=1; flag=3;
      if k==1
         move=0; birth=1;flag=1; % just re-draw coefficient
      end
   end   
   ...
   if birth
      ...
      curVarIdx_prop=curVarIdx;
      [X_prop(:,k_prop) Xt_prop(:,k_prop) basis_prop,curVarIdx_prop(k_prop)]=gen_mars_basis5(X,Xt,interaction(indx_d),order(indx_o),mnmx,curVarIdx(1:k));
      if SAVE_SAMPLES
  ...
  elseif death
      ...
      Xt_prop(:,indx)=[];
      curVarIdx_prop=curVarIdx;
      curVarIdx_prop(indx:(end-1))=curVarIdx_prop(indx+1:end);      
      if SAVE_SAMPLES
   ...
   elseif move 
      ...
      curVarIdx_prop=curVarIdx;
      % update design matrix
      [X_prop(:,indx) Xt_prop(:,indx) basis_prop,curVarIdx_prop(indx)]=gen_mars_basis5(X,Xt,interaction(indx_d),order(indx_o),mnmx,curVarIdx(1:k));
      if SAVE_SAMPLES
   ...
   if count>burn_in
      ...
      %added by Kaiguang Zhao
%       for ii=2:k
%           varT=model.basis(ii).var;
%           if length(varT) == 1
%               probMatrix(varT,varT)=probMatrix(varT,varT)+1;
%           else
%               probMatrix(varT(1),varT(2))=probMatrix(varT(1),varT(2))+1;
%           end
%      
%       end
%               
...
%
%
function [x, xt, basis,chosenIdx]=gen_mars_basis5(X,Xt,interaction,order,mnmx,curVarIdx);
...
while not_finished
       ...
       for j=1:depth
          ...
          while not_ok
             ...
             if ~ismember(ind,curVarIdx)
                var(j)=ind;
                curVarIdx=[curVarIdx,var(j)];
                chosenIdx=var(j);
                not_ok=0;
             end
          end
...
%% modifed by kaiguang
       ...
       % put to power
       if order==0
          temp(:,j) = double(temp(:,j)~=0);
          if test
             temp_t(:,j) = double(temp_t(:,j)~=0);
          end
       elseif order == -1
          temp(:,j) = X(:,var(j));
          if test
       ...
       elseif order == -2
          temp(:,j) = log(abs(temp(:,j))+1);
          if test
            temp_t(:,j) = log(abs(temp_t(:,j))+1);
          end
       elseif order == 1
         temp(:,j) = temp(:,j);
          if test
          ...
          temp(:,j)=abs(temp(:,j)).^order;
          if test
               temp_t(:,j)=abs(temp_t(:,j)).^order;
          end
       end
    end
     %%
